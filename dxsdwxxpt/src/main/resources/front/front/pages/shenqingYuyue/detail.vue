	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.shenqingYuyueId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('shenqingYuyue', this.shenqingYuyueId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.shenqingYuyuePhoto ? this.detail.shenqingYuyuePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('shenqingYuyueYuyue', {

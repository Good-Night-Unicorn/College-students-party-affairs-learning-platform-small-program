	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.shenqingYuyueText" type="text" placeholder="申请理由"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('shenqingYuyue','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('shenqingYuyue','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='shenqingYuyueTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in shenqingYuyueTypesList" :key="index" :class="shenqingYuyueTypes===item.codeIndex?'tabActive':''"
                                  @tap="shenqingYuyueTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.shenqingYuyueName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('shenqingYuyue','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                ...
                <!-- 当前表 -->
                 <view v-show="item.shenqingYuyueUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名编号</text>
                     <text style="width: 70%">
                         <text v-if="item.shenqingYuyueUuidNumber != null">{{item.shenqingYuyueUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.shenqingYuyueText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">申请理由</text>
                    <view style="width: 70%" v-html="item.shenqingYuyueText"></view>
                </view>
                 <view v-show="item.shenqingYuyueFile" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">上传图片</text>
                     <view style="width: 70%" @tap="download(baseUrl+item.shenqingYuyueFile)" class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>下载</view>
                 </view>
                 <view v-show="item.shenqingYuyueYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名状态</text>
                     <text style="width: 70%">{{item.shenqingYuyueYesnoValue}}</text>
                 </view>
                 <view v-show="item.shenqingYuyueValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">申请状态</text>
                     <text style="width: 70%">{{item.shenqingYuyueValue}}</text>
                 </view>
                <view v-show="item.shenqingYuyueYesnoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核回复</text>
                    <view style="width: 70%" v-html="item.shenqingYuyueYesnoText"></view>
                </view>
                <view v-show="item.shenqingYuyueShenheTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">审核时间</text>
                    <text style="width: 70%">
                        <text v-if="item.shenqingYuyueShenheTime != null">{{item.shenqingYuyueShenheTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
                <view v-show="item.shenqingYuyueTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">预约时间</text>
                    <text style="width: 70%">
                        <text v-if="item.shenqingYuyueTime != null">{{item.shenqingYuyueTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('shenqingYuyue','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('shenqingYuyue','审核') && item.shenqingYuyueYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 申请状态 字段 字典表查询方法
            async shenqingYuyueTypesSelect(){
                let shenqingYuyueTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'shenqing_yuyue_types',
                });
                shenqingYuyueTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.shenqingYuyueTypesList = shenqingYuyueTypes.data.list
            },
            //当前表的 申请状态字段类别搜索
			shenqingYuyueTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.shenqingYuyueTypes != 0) {
                    params.shenqingYuyueTypes = this.shenqingYuyueTypes
                }
                let _this = this
                let res = await this.$api.page(`shenqingYuyue`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('shenqingYuyue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`shenqingYuyue`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.shenqingYuyueUuidNumber) {
                    searchForm['shenqingYuyueUuidNumber'] = this.searchForm.shenqingYuyueUuidNumber
                }
				if (this.searchForm.shenqingYuyueText) {
                    searchForm['shenqingYuyueText'] = this.searchForm.shenqingYuyueText
                }

                let res = await this.$api.page(`shenqingYuyue`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空

	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.zuoyeTijiaoText" type="text" placeholder="内容"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('zuoyeTijiao','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('zuoyeTijiao','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.zuoyeTijiaoName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('zuoyeTijiao','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <view v-show="item.zuoyeName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">作业名称</text>
                    <text style="width: 70%">{{item.zuoyeName}}</text>
                </view>
                <view v-show="item.zuoyeTypes" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">作业类型</text>
                    <text style="width: 70%">{{item.zuoyeValue}}</text>
                </view>
                <view v-show="item.gongzuoName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">党务工作者姓名</text>
                    <text style="width: 70%">{{item.gongzuoName}}</text>
                </view>
                ...
                <!-- 当前表 -->
                 <view v-show="item.zuoyeTijiaoUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">报名唯一编号</text>
                     <text style="width: 70%">
                         <text v-if="item.zuoyeTijiaoUuidNumber != null">{{item.zuoyeTijiaoUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.zuoyeTijiaoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">内容</text>
                    <view style="width: 70%" v-html="item.zuoyeTijiaoText"></view>
                </view>
                 <view v-show="item.zuoyeShangchuanFile" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">作业上传</text>
                     <view style="width: 70%" @tap="download(baseUrl+item.zuoyeShangchuanFile)" class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>下载</view>
                 </view>
                 <view v-show="item.zuoyeTijiaoYesnoValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">提交状态</text>
                     <text style="width: 70%">{{item.zuoyeTijiaoYesnoValue}}</text>
                 </view>
                 <view v-show="item.zuoyeTijiaoPigai" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">批改分数</text>
                     <text style="width: 70%">{{item.zuoyeTijiaoPigai}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('zuoyeTijiao','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('zuoyeTijiao','审核') && item.zuoyeTijiaoYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`zuoyeTijiao`, params);
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`zuoyeTijiao`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.zuoyeTijiaoUuidNumber) {
                    searchForm['zuoyeTijiaoUuidNumber'] = this.searchForm.zuoyeTijiaoUuidNumber
                }
				if (this.searchForm.zuoyeTijiaoText) {
                    searchForm['zuoyeTijiaoText'] = this.searchForm.zuoyeTijiaoText
                }

                let res = await this.$api.page(`zuoyeTijiao`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空

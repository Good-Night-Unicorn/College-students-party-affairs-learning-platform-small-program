	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        ...
        <view class="action">
            ...
            <button
                    ...
                    v-if="isAuth('zuoyeTijiao','新增')" @click="onAddTap()" class="uniui-add">新增</button>
		</view>
	</view>
            ...
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                 ...
                 <view v-show="item.zuoyeTijiaoUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <text style="width: 70%">{{item.zuoyeTijiaoUuidNumber}}</text>
                 </view>
                 <view v-show="item.zuoyeTijiaoText" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <text style="width: 70%">{{item.zuoyeTijiaoText}}</text>
                 </view>
                 <view v-show="item.zuoyeShangchuanFile" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     ...
                     <view style="width: 70%" @tap="download(item.zuoyeShangchuanFile)" class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>下载</view>
                 </view>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
		  	...
		  	let zuoyeTijiaoYesnoTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'zuoye_tijiao_yesno_types'
		  	});
		  	this.zuoyeTijiaoYesnoTypesList = zuoyeTijiaoYesnoTypes.data.list;
            ...
            wuyong(id) {
                ...
                uni.showModal({ content: '确定要 么?',title: '提示',
                    success: async function(res) {
                        if (res.confirm) {
                            await _this.$api.update(`zuoyeTijiao`, {
                                id:id,
                            });

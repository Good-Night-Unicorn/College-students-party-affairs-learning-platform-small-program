	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xuexiCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('xuexiCollection', this.xuexiCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xuexiCollectionPhoto ? this.detail.xuexiCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xuexiCollectionYuyue', {

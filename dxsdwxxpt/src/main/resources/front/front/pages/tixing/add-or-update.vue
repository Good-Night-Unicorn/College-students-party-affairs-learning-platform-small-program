        ...
        <form class="app-update-pv">
            ...
            <view
                ...
                <textarea :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx","height":"200rpx"}'
                          :disabled="ro.tixingText" v-model="ruleForm.tixingText" placeholder="留言内容"/>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`tixing`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.tixingId){
                this.ruleForm.tixingId = options.tixingId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
				if ((!this.ruleForm.tixingText)) {
                    this.$utils.msg(`留言内容不能为空`);
                    return
                }
                if (this.ruleForm.id) {
                    await this.$api.update(`tixing`, this.ruleForm);
                } else {
                    await this.$api.save(`tixing`, this.ruleForm);
                }

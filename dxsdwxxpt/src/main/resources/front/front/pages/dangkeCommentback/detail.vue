	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dangkeCommentbackId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dangkeCommentback', this.dangkeCommentbackId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dangkeCommentbackPhoto ? this.detail.dangkeCommentbackPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dangkeCommentbackYuyue', {

        ...
        <form class="app-update-pv">
			<view
                ...
                <textarea
                        ...
                        v-model='ruleForm.xuexiLiuyanText' placeholder="留言内容"></textarea>
            </view>
    ...
    export default {
        ...
        async onLoad(options) {
            ...
            if (options.id) {
                ...
                // 获取信息
                let res = await this.$api.info(`xuexiLiuyan`, this.ruleForm.id);
                this.ruleForm = res.data;
            }
            if(options.xuexiId){
                this.ruleForm.xuexiId = options.xuexiId;
            }
            // 跨表
            // this.styleChange()
        },
        methods: {
            ...
            async onSubmitTap() {
			if ((!this.ruleForm.xuexiLiuyanText)) {
                this.$utils.msg(`留言内容不能为空`);
                return
            }
                if (this.ruleForm.id) {
                    await this.$api.update(`xuexiLiuyan`, this.ruleForm);
                } else {
                    await this.$api.save(`xuexiLiuyan`, this.ruleForm);
                }

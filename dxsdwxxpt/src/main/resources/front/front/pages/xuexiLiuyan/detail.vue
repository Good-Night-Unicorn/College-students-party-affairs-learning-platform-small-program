	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('xuexiLiuyan', this.xuexiLiuyanId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.xuexiLiuyanPhoto ? this.detail.xuexiLiuyanPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xuexiLiuyanYuyue', {

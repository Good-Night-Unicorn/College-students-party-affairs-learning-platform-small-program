	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.dangkeCollectionId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dangkeCollection', this.dangkeCollectionId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dangkeCollectionPhoto ? this.detail.dangkeCollectionPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dangkeCollectionYuyue', {

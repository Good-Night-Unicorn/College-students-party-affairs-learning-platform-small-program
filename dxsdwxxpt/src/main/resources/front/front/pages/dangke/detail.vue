		...
		<view class="container">
			...
			<view class="">
				...
				<view class="yinyuewai">
					<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.dangkeName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				...
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.dangkeContent"></rich-text>
						</view>
					</view>
				</view>
				<!-- 评价 -->
...
<script>
	export default {
		...
		async onLoad(options) {
			this.dangkeId = options.id;
		},
		async onShow(options) {
			// 渲染数据
			this.init();
			//获取收藏信息
			this.getdangkeCollection()
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.detail('dangke', this.dangkeId);
				this.detail = res.data
				this.detail.dangkeContent = this.detail.dangkeContent.replace("<img", "<img  class=\"daxiao\"")
					.replace("src=\"upload/", "src=\"" + this.$base.url + "upload/");
				// 主图
				this.swiperList = this.detail.dangkePhoto ? this.detail.dangkePhoto.split(",") : [];
			},
			...
			// 收藏
			async getdangkeCollection() {
				let params = {
					page: 1,
					limit: 1,
					dangkeId: this.dangkeId,
			...
			async shoucang() {
				...
				let res = await _this.$api.list(`dangkeCollection`, params);
				if (res.data.list.length == 1) {
					let dangkeCollectionId = res.data.list[0].id;
					uni.showModal({
						title: '提示',
						content: '是否取消',
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.del('dangkeCollection', JSON.stringify([
									dangkeCollectionId]));
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('dangkeCollection', {
								yonghuId: _this.user.id,
								dangkeId: _this.detail.id,
								dangkeCollectionTypes: 1
							});
			...
			async upCallback(mescroll) {
				let res = await this.$api.list('dangkeCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					dangkeId: this.dangkeId
				});
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								...
								await _this.$api.update('dangke', _this.detail);
								_this.$utils.msg('取消成功');
			...
			async baoming() {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.add('dangkeYuyue', {
								yonghuId: _this.user.id,
								dangkeId: _this.detail.id,
							});

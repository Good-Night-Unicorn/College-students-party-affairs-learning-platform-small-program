	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.dangkeName" type="text" placeholder="党课名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('dangke','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('dangke','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='dangkeTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in dangkeTypesList" :key="index" :class="dangkeTypes===item.codeIndex?'tabActive':''"
                                  @tap="dangkeTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <view class="list"
        ...
        <view @tap="onDetailTap(product)" class="listm flex flex-between"
            ...
            <image
                    ...
                    :src="product.dangkePhoto?baseUrl+product.dangkePhoto.split(',')[0]:''"></image>
            <view class="listmr">
                <view class="hide2"
                ...
                <view v-if="product.dangkeNewMoney"
                    ...
                    ￥{{product.dangkeNewMoney}}
                </view>
                <view v-else-if="product.dangkeValue"
                      :style='{"padding":"10rpx 10rpx 0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0 auto","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                    {{product.dangkeValue}}
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('dangke','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('dangke','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 党课类型 字段 字典表查询方法
            async dangkeTypesSelect(){
                let dangkeTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'dangke_types',
                });
                dangkeTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.dangkeTypesList = dangkeTypes.data.list
            },
            //当前表的 党课类型字段类别搜索
			dangkeTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.dangkeTypes != 0) {
                    params.dangkeTypes = this.dangkeTypes
                }
                let _this = this
                let res = await this.$api.list(`dangke`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('dangke', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`dangke`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.dangkeName) {
                    searchForm['dangkeName'] = this.searchForm.dangkeName
                }

                let res = await this.$api.list(`dangke`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.dangkeContent!=null){
                        item.dangkeContent = item.dangkeContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");

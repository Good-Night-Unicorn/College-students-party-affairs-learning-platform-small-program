	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('dangkeOrder', this.dangkeOrderId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.dangkeOrderPhoto ? this.detail.dangkeOrderPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('dangkeOrderYuyue', {
									yonghuId: _this.user.id,
									dangkeOrderId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/dangkeOrderYuyue/list');
							}
						}
					});
				},

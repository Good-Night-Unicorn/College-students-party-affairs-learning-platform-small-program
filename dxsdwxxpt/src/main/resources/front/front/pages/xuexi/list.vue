	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 30rpx","boxShadow":"0 0 16rpx var(--publicSubColor) inset","backgroundColor":"rgba(255, 255, 255, 1)","borderColor":"var(--publicMainColor)","borderRadius":"20rpx","color":"var(--publicMainColor)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx","marginTop":"7rpx"}'
						v-model="searchForm.xuexiName" type="text" placeholder="在线学习名称"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('xuexi','新增')" @click="onAddTap()"
            ...
            <view v-if="isAuth('xuexi','报表')" @tap="openChartDialog" :style="{padding:'0 10rpx',lineHeight:'80rpx',width:'auto',height:'80rpx',fontSize:'28rpx',color:'rgba(239, 239, 239, 1)',backgroundColor:btnColor[0],borderColor:btnColor[0]}">
                报表
            </view>
                ...
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;">
                        <view style="display: flex;flex-direction: column;width: 100%;">
                            <view :style='xuexiTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                                  v-for="(item,index) in xuexiTypesList" :key="index" :class="xuexiTypes===item.codeIndex?'tabActive':''"
                                  @tap="xuexiTypesCategoryClick(item.codeIndex)">
                                {{item.indexName}}
                            </view>
                        </view>
                    </view>
    ...
    <view class="list"
        ...
        <view @tap="onDetailTap(product)" class="listm flex flex-between"
            ...
            <image
                    ...
                    :src="product.xuexiPhoto?baseUrl+product.xuexiPhoto.split(',')[0]:''"></image>
            <view class="listmr">
                <view class="hide2"
                ...
                <view v-if="product.xuexiNewMoney"
                    ...
                    ￥{{product.xuexiNewMoney}}
                </view>
                <view v-else-if="product.xuexiValue"
                      :style='{"padding":"10rpx 10rpx 0","boxShadow":"0 2rpx 12rpx rgba(0,0,0,0)","margin":"0 auto","borderColor":"rgba(0,0,0,0)","backgroundColor":"rgba(0,0,0,0)","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}'>
                    {{product.xuexiValue}}
                </view>
                <view style="display: flex;justify-content: space-between;">
                    <text v-if="isAuth('xuexi','修改')" class="cuIcon-edit"
                          @click.stop="onUpdateTap(product.id)">修改
                    </text>
                    <text v-if="isAuth('xuexi','删除')" class="cuIcon-delete"
...
<script>
	export default {
		...
		methods: {
            ...
            //当前表的 在线学习类型 字段 字典表查询方法
            async xuexiTypesSelect(){
                let xuexiTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'xuexi_types',
                });
                xuexiTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.xuexiTypesList = xuexiTypes.data.list
            },
            //当前表的 在线学习类型字段类别搜索
			xuexiTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xuexiTypes != 0) {
                    params.xuexiTypes = this.xuexiTypes
                }
                let _this = this
                let res = await this.$api.list(`xuexi`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xuexi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            await _this.$api.update(`xuexi`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.xuexiName) {
                    searchForm['xuexiName'] = this.searchForm.xuexiName
                }

                let res = await this.$api.list(`xuexi`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.xuexiContent!=null){
                        item.xuexiContent = item.xuexiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");

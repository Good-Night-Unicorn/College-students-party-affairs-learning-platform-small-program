			...
			<view class="yinyuewai">
				<view class="yinyuenei"><!-- @tap="onErweima()" -->
						...
						<view class="shangpinfei">
							{{detail.xuexiName}}<!-- <uni-icons type="scan" size="30"></uni-icons> -->
						</view>
				<view class="xiangqing">
					<view class="shangpinxiangqing">
						...
						<view class="app">
							{{detail.xuexiUuidNumber}}
						</view>
					</view>
				</view>
				<view class="xiangqing" @tap="download(baseUrl + detail.xuexiFile)">
					<view class="shangpinxiangqing">
						<view class="box">
							文件：
						</view>
					...
					<view class="yinyuenei uni-padding-wrap uni-common-mt">
						<view>
							<video id="myVideo" style="preload:auto;margin: 20rpx;border-radius: 20rpx;" muted
								   :src="baseUrl+detail.xuexiVideo" controls></video>
						</view>
					</view>
				</view>
				<!-- 简介 -->
				<view class="yinyuewai">
					...
					margin-bottom: 25rpx;
					text-indent:24px">
							<rich-text :nodes="detail.xuexiContent"></rich-text>
						</view>
					</view>
				</view>
            <!-- 留言 -->
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xuexiId = options.id;
			},
			async onShow(options) {
				// 渲染数据
				this.init();
				//获取收藏信息
				this.getxuexiCollection()
			...
			methods: {
                ...
                async submitData() {
                    ...
                    let data = {
                        xuexiId: this.xuexiId,
                        yonghuId: this.user.yonghuId,
						xuexiLiuyanPingfenNumber: this.scoreData,
                        xuexiLiuyanText: this.commentData,
                    }
                    await this.$api.save(`xuexiLiuyan`, data);
                    this.$refs.deliver.close()
                    this.mescroll.num = 1
                    this.upCallback(this.mescroll)
                },
				...
				// 获取详情
				async init() {
					let res = await this.$api.detail('xuexi', this.xuexiId);
                    this.detail = res.data
					this.detail.xuexiContent = this.detail.xuexiContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.xuexiPhoto ? this.detail.xuexiPhoto.split(",") : [];
				},
				/* && !nameList?seq_contains('zuowei_number')*/
				// 收藏
				async getxuexiCollection() {
					let params = {
						page: 1,
						limit: 1,
						xuexiId: this.xuexiId,
				...
				async shoucang() {
					...
					let res = await _this.$api.list(`xuexiCollection`, params);
					if (res.data.list.length == 1) {
						let xuexiCollectionId = res.data.list[0].id;
						uni.showModal({
							title: '提示',
							content: '是否取消',
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('xuexiCollection', JSON.stringify([xuexiCollectionId]));
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xuexiCollection', {
									yonghuId: _this.user.id,
									xuexiId: _this.detail.id,
									xuexiCollectionTypes:1
								});
				...
				async upCallback(mescroll) {
					let res = await this.$api.list('xuexiLiuyan', {
						page: mescroll.num,
						limit: mescroll.size,
						xuexiId: this.xuexiId
					});
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									...
									await _this.$api.update('xuexi', _this.detail);
									_this.$utils.msg('取消成功');
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xuexiYuyue', {
									yonghuId: _this.user.id,
									xuexiId: _this.detail.id,
								});

                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('tixing','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/dxsdwxxpt/upload/tixingMuBan.xls"
                        >批量导入提醒数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('tixing','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="tixingUploadSuccess"
                                :on-error="tixingUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('tixing','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "tixing.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="留言内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.tixingText != null &&scope.row.tixingText.length>10">
                                {{scope.row.tixingText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.tixingText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('tixing','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('tixing','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('tixing','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'tixing',//当前表表名,
                        sumColum : 'tixing_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'tixing_name',//分组字符串字段
                        // types : 'tixing_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['tixingDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            tixingUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "tixing/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({

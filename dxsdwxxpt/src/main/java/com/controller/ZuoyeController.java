    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(zuoye !=null){
            ...
            //级联表 党课
            //级联表
            DangkeEntity dangke = dangkeService.selectById(zuoye.getDangkeId());
            if(dangke != null){
            BeanUtils.copyProperties( dangke , view ,new String[]{ "id", "createTime", "insertTime", "updateTime"});//把级联的数据添加到view中,并排除id和创建时间字段,当前表的级联注册表
    ...
    */
    ...
    public R save(@RequestBody ZuoyeEntity zuoye, HttpServletRequest request){
        ...
        Wrapper<ZuoyeEntity> queryWrapper = new EntityWrapper<ZuoyeEntity>()
            .eq("dangke_id", zuoye.getDangkeId())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            ZuoyeEntity zuoyeEntity = new ZuoyeEntity();
//                            zuoyeEntity.setDangkeId(Integer.valueOf(data.get(0)));   //党课 要改的
